...
function loader() {
	...
	xmlhttp.onload = function() {
		...
		for (let x in myObj) {
			text += "<tr><td>" + myObj[x].id + "</td><td>" + myObj[x].first_name + "</td><td>" + myObj[x].last_name + "</td><td>" + myObj[x].email + "</td><td>" + myObj[x].age + "</td><td>" + myObj[x].gender + "</td><td>" + myObj[x].is_maried + "</td><td>" + myObj[x].country + "</td><td>" + myObj[x].ballance_usd;
			//dodawanie do tabeli
...
function dataprep(countries) {
	...
	while (i1 < counters.length) // sortowanie 
	{
		while (i2 < (counters.length - 1)) {
			if (counters[i2] < counters[i2 + 1]) {
				temp1 = counters[i2];
...
// wykresy 
function charts(unique, counters) {
	...
	const myChart = new Chart(ctx, {
		...
		data: {
			labels: unique,
			datasets: [{
				label: 'Poniższe wykresy przedstawiają 5 najczęściej występujących krajów w powyższym zbiorze danych. "Other" odwołuje się do sumy wystąpień pozostałych krajów.',
				data: counters,
				backgroundColor: [
